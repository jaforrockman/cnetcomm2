...
class CostsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create cost" do
    assert_difference('Cost.count') do
      post costs_url, params: { cost: { cost_amount: @cost.cost_amount, date: @cost.date, kind: @cost.kind, name: @cost.name } }
    end
  ...
  test "should update cost" do
    patch cost_url(@cost), params: { cost: { cost_amount: @cost.cost_amount, date: @cost.date, kind: @cost.kind, name: @cost.name } }
